<template>
  <div id="taskdetail">
    ...
    <div class="unfinished" style="padding: 5px 5px; margin-top: 10px">
      <div style="font-weight: bold; text-align: left; color: coral;padding-left: 5px; font-size: 20px">未完成任务</div>
      ...
      <table class="am-table am-table-bordered">
        ...
        <tbody>
          <tr v-for="item in unfinishedlist" track-by="$index">
            ...
            <td v-show="unfinishedlistcolormark[$index] === 0 && item.countdownstate === 1" style="color: deepskyblue;background-color: lightskyblue">今天</td>
    ...
    <div class="finished" style="padding: 5px 5px">
      <div style="font-weight: bold; text-align: left; color: coral;padding-left: 5px; font-size: 20px">已完成任务</div>
      <hr data-am-widget="divider" style="margin:2px 0 8px 0;padding:0;width: 50%" class="am-divider am-divider-default" />
      <table class="am-table am-table-bordered">
        <thead>
        <tr>
  ...
  export default{
    ...
    computed: {
      ...
      finishedlist: function () {
        console.log(this.$root.currentread.finishedlist)
    ...
    methods: {
      review (index) {  // 查看已完成问卷
        ...
        this.$router.go('/withdrawtask')
      },
      settask () {  // 设置任务属性
        this.$router.go('/settask')
      }
    }
  }
</script>
