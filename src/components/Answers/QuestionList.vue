    ...
    <div v-if="questionItem.type === 'blank' || questionItem.type === 'select' || questionItem.type === 'multi_select' ||
      ...
      <form class= "am-form" style="padding:0;margin:0">
        ...
        <div v-if="questionItem.type === 'symptom_score'">
          <Score :question-item = "questionItem" v-ref:score></Score>
        </div>
      </form>
    </div>
    <div v-if="questionItem.type === 'upload_image'">
      <uploadimg v-ref:upimg></uploadimg>
    </div>
  </div>
</template>
<style lang="scss">
    ...
    export default{
      ...
      methods: {
        ...
        dispatchAnswer (questionItem) {
          ...
          this.$dispatch('thisanswerstate', status)  // 把答案的state传给父节点
          this.$dispatch('thisanswer', data)  // 把当前的答案以data格式传送给父节点
        }
      }
    }
</script>
